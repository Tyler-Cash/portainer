...
Using this fast local volume (rather than `/hdd` or `/ssd/services/...`) matters here because raw source files can be multiple GB and both the upload write and the ffmpeg read/write happen against it directly.
...
- **Byte-range bug fix carried in the same pass:** the original Range-request handling didn't clamp `end` to the file size or reject `start >= size`, so a request near end-of-file could advertise a `Content-Length` larger than the bytes actually sent — the browser/`mpegts.js` would then hang waiting for bytes that never arrived. Fixed via `lib/range.ts`'s `resolveRange()`, unit tested directly. **Superseded in v1.2** — see below; this route no longer serves raw bytes at all.
...
## v1.2: server-side remux replaces client-side MSE/mpegts.js entirely

Real-world testing with large HEVC `.ts` source files surfaced a chain of MSE/`mpegts.js`-side bugs (non-finite `duration` on `loadedmetadata`, an `ERR_INVALID_STATE` uncaught exception when a client aborts a range request mid-stream, and — the actual root cause of "plays for a while then hangs forever" — `mpegts.js`'s `autoCleanupSourceBuffer` defaulting to `false`, so the browser's `SourceBuffer` fills up and is never evicted). Rather than keep patching individual MSE edge cases, the preview pipeline was rebuilt around a different architecture:

- **Server-side live remux, not client-side demux.** `GET /api/upload/[id]` no longer serves raw source bytes with Range support. It now accepts an optional `?start=<seconds>` query param, spawns `ffmpeg -ss <start> -i <source> -c copy -avoid_negative_ts make_zero -movflags frag_keyframe+empty_moov+default_base_moof -f mp4 pipe:1` (`lib/remux.ts`), and streams `stdout` straight into the HTTP response via `lib/streams.ts`'s `nodeStreamToResponseStream()`. `-c copy` means this is repackaging, not re-encoding — cheap, no quality loss, and codec-agnostic (works for HEVC, H.264, whatever ffmpeg can read), which is what makes "support everything ffmpeg supports" possible without a browser-compatibility allowlist. The frontend points a plain `<video>` at this — no MSE, no `SourceBuffer`, no `mpegts.js` dependency at all anymore.
- **Seeking restarts the remux, it doesn't Range-request into it.** A live pipe has no fixed byte length, so native Range-based seeking doesn't apply. Instead, `seekTo(time)` in `page.tsx` reassigns `video.src` to `/api/upload/[id]?start=<time>` and calls `.load()`, which aborts the in-flight fetch (triggering `ReadableStream.cancel()` server-side, which kills the ffmpeg child process via an `onCancel` callback on `nodeStreamToResponseStream` — otherwise every seek would leak an orphaned ffmpeg process) and starts a fresh remux from that point. Since the new sub-stream's own internal timestamps start near 0, the frontend tracks a `streamOffset` (the point the current sub-stream started from) and computes the effective absolute position as `streamOffset + video.currentTime` everywhere (playhead display, "Start/Stop clip here", etc.).
- **`nodeStreamToResponseStream()` (`lib/streams.ts`) replaces `Readable.toWeb()`.** The latter has a race where its internal `'end'`/`'data'` handlers can call `controller.close()`/`.enqueue()` after the consumer already canceled it, throwing uncaught. The replacement wraps every controller call in the same file in a try/catch instead, and is shared by both the video remux route and (implicitly, via the same pattern) anywhere else a Node stream needs to become a Response body.
- **Duration comes from `ffprobe`, not the video element.** `POST /api/upload` now runs `lib/ffprobe.ts`'s `getDuration()` right after the upload lands on disk and returns `{ id, duration }`. The frontend uses that directly instead of `onLoadedMetadata`/`ondurationchange` — which structurally eliminates the non-finite-duration bug class rather than just guarding against it, since a live fragmented-mp4 pipe with an empty `moov` never reports a real duration through the video element anyway.
- **Broadened accepted formats.** `ACCEPTED_EXTENSIONS` in `lib/paths.ts` is no longer "must be browser-playable" (`.ts`, `.mp4`, `.webm`, `.m4v`) — since remuxing happens server-side regardless of source format, it's now a broad sanity check covering common containers ffmpeg reads (adds `.mov`, `.mkv`, `.avi`, `.flv`, `.wmv`, `.mpg`, `.mpeg`).
- **Thumbnail preview while dragging (`lib/thumbnail.ts`, `GET /api/upload/[id]/thumbnail?t=<seconds>`).** Generated on demand (not pre-batched for the whole video) via a single-frame `ffmpeg -ss <t> -frames:v 1` extract, cached to disk keyed by id + rounded second so repeated drags near the same position are a disk read, not a new ffmpeg invocation. Cache files are cleaned up alongside the source on `DELETE /api/upload/[id]` (`removeThumbnails()`) and by the existing 24h sweep otherwise.
- **Playbar UI (`app/playbar.tsx`, supersedes `app/timeline.tsx`).** Adds a play/pause button synced to the video's own `play`/`pause` events; the start/end drag handles are always present but only visually emphasized on hover (`.timeline:hover .timeline-handle`) so they don't clutter the UI at rest; dragging a handle shows a floating thumbnail preview (`.timeline-preview`) positioned above the cursor, sourced from the on-demand thumbnail endpoint; a small delete-cross appears near the end handle on hover to discard the current draft selection (collapses `draftEnd` to `draftStart`, which the existing `addToQueue` zero-length guard already treats as "nothing to add").
- **Removed as dead code:** `lib/range.ts` (`resolveRange`), `mimeTypeFor` (`lib/paths.ts`), and the `mpegts.js` npm dependency — nothing in the new architecture uses any of them.
