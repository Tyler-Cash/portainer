...
| `stacks/spoolman/scripts/sync-ha-package.sh` (create) | Pulls the generated HA config into the repo |
| `Taskfile.yml` (modify) | `task spoolman:sync-package` target |
...
services:
  ...
  # Assigns spools to printer slots and deducts usage. Reads printer state from
  # Home Assistant via the ha-bambulab integration — it does not talk to the
  # printer directly. The deduction automations live in
  ...
  spoolmansync:
    ...
    environment:
      ...
      # OAuth redirect base for the Home Assistant login handshake — must be the
      # external URL, not the container name.
...
- [ ] **Step 3: Generate and write the package**

```bash
task spoolman:sync-package
```

This calls `POST /api/automations {"action":"discover"}` and writes
`stacks/home-assistant/packages/spoolmansync.yaml`. It refuses to run if SpoolmanSync is
not connected to HA, and warns loudly if any Home Assistant entity is missing — a missing
`print_weight` breaks usage tracking silently, which is the exact failure this setup
exists to prevent.

**Why not copy-paste from the UI:** SpoolmanSync's Automations page shows two blocks, and
its own auto-configure writes them to *two different files* — `configurationYaml` (the
`rest_command` / `input_number` / `utility_meter` / `sensor` block) goes to the package
file, while `automationsYaml` is merged into HA's UI-managed `automations.yaml`, which
this repo does not track. Pasting only the automations block into the package file yields
a package with no `rest_command` and automations HA never loads. The script folds both
into the one git-managed file, which works because HA packages accept `automation:` as a
top-level key.

- [ ] **Step 4: Verify it parses and defines what it should**
...
Expected: top-level keys including `rest_command` and `automation`, at least one
`spoolmansync_*` rest command, and a non-zero automation count.

HA's `!include` and `!secret` tags would break `yaml.safe_load`. If the generated file
contains them, that is a real problem worth investigating rather than working around,
because this package is meant to be self-contained.
