...
## Spool Management (Bambu P1S)

Filament inventory lives in Spoolman (`spoolman.tylercash.dev`), with SpoolmanSync (`spools.tylercash.dev`) assigning the loaded spool and deducting usage. Both are in `stacks/spoolman/`. Design: `docs/superpowers/specs/2026-08-06-spool-management-design.md`.

SpoolmanSync does not talk to the printer — it reads state from Home Assistant via the `ha-bambulab` integration and drives Spoolman's REST API through HA automations. **Those automations live at `stacks/home-assistant/packages/spoolmansync.yaml`, which is generated by SpoolmanSync's Automations page. Never hand-edit it** — regenerate there and re-copy, or the next regeneration silently drops your changes.

The package is bind-mounted read-only at `/config/packages`, and `configuration.yaml` (on the host, not in git) carries the matching `packages: !include_dir_named packages` directive. The `home-assistant` service sets `STACK_CONTENT_HASH` so editing the package recreates the container and reloads it.

The printer has **no AMS**, only the external spool holder, so there is no RFID reader and no auto-detection of what is loaded. The spool must be assigned by hand (or QR scan) on every swap, and deducted weight comes from the slicer's estimate rather than a measurement — re-weigh spools occasionally and correct the value in Spoolman.
